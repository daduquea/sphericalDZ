...
class SphericalProjector(AbstractProjector):
    ...
    def project_point(self, points):
        ...
        #print("FOV", self.proj_fov_up, self.proj_fov_down)     
...
class Projection:
    ...
    def project_points_values(self, points, values, aggregate_func='max', layers=None):
        ...
        return binned_values_map, unq_ids
...
def back_proj_front_pred(cloud, pred, proj, inference='regression', layers = None):
    """
    Parameters
    ----------
    cloud: Pyntcloud
...
# DAVID
def back_proj_front_predicted(xyz_points, pred, proj, inference='regression', layers = None, ground_labels = None):
    """
    Parameters
    ----------
    cloud: Pyntcloud
    ...
    # project gt
    acc_img = proj.project_points_values(xyz_points, np.ones_like(xyz_points[:,0]), aggregate_func='sum', layers=layers)

    # back project gt
    back_proj_acc_mask = proj.back_project(xyz_points, acc_img == 1, res_value=1, layers=layers)
    back_proj_pred = proj.back_project(xyz_points, pred, res_value=1, layers=layers)
    ...
    X_train, y_train = xyz_points[back_proj_acc_mask == 1, :], back_proj_pred[back_proj_acc_mask == 1]
    X_test = xyz_points[back_proj_acc_mask != 1, :]
